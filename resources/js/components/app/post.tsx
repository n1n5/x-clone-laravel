...
export function Post({ post }: PostProps) {
    ...
    const getImageSrc = (path: string): string => (path.startsWith('http') ? path : `/${path}`);

    return (
        <div className="border-y-[1px] border-borderCustom p-4">
            {isRepost && <RepostHeader user={post.user} />}
...
function PostAvatar({ user, getImageSrc }: { user: PostType['user']; getImageSrc: (path: string) => string }) {
    return (
        <Avatar>
            <AvatarImage src={user.avatar_path ? getImageSrc(user.avatar_path) : '/icons/profile.svg'} alt={user.username} height={100} width={100} />
        </Avatar>
    );
}
...
function PostContent({ post, getImageSrc }: { post: PostType; getImageSrc: (path: string) => string }) {
    return (
        <Link href={route('post.show', { post: post.id })}>
            <p className="text-textDarkMode">{post.body}</p>
            {post.attachments && post.attachments.length > 0 && <PostAttachments attachments={post.attachments} getImageSrc={getImageSrc} />}
        </Link>
    );
}

function PostAttachments({ attachments, getImageSrc }: { attachments: PostType['attachments']; getImageSrc: (path: string) => string }) {
    return (
        <div>
